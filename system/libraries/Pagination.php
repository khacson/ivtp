...
class CI_Pagination {
	...
	function create_links_ex($cur_page=0)
	{
		// If our item count or per-page total is zero there is no need to continue.
		if ($this->total_rows == 0 OR $this->per_page == 0)
		{
		...
		{
			$i = $uri_page_number - $this->per_page;
			$output .= $this->prev_tag_open.'<a href="'.$this->base_url.$i.'" name="'.$i.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
		}
		else
		{
			$healthy = array("button2-right");
		...
		for ($loop = $start -1; $loop <= $end; $loop++)
		{
			...
			if ($i >= 0)
			{
				...
				{
					if ($i == 0) {
						$output .= $this->num_tag_open.'<a href="'.$this->base_url.'" title="Trang '.$loop.'" name="'.$i.'">'.$loop.'</a>'.$this->num_tag_close;
					}
					else {
						$output .= $this->num_tag_open.'<a href="'.$this->base_url.$i.'" title="Trang '.$loop.'" name="'.$i.'">'.$loop.'</a>'.$this->num_tag_close;
					}
		...
		if ($this->cur_page < $num_pages)
		{
			$next_page = $this->cur_page * $this->per_page;
			$output .= $this->next_tag_open.'<a href="'.$this->base_url.$next_page.'" name="'.$next_page.'">'.$this->next_link.'</a>'.$this->next_tag_close;
		}
		else
		{
			$healthy = array("button2-left");
		...
		if($num_pages>3)
		{
			if ($this->cur_page < $num_pages)
			{
				...
				$i = (($num_pages * $this->per_page) - $this->per_page);
				$output .= $this->last_tag_open.'<a href="'.$this->base_url.$i.'" name="'.$i.'">'.$this->last_link.'</a>'.$this->last_tag_close;
			}
			else
			{
				$healthy = array("button2-left");
