...
class base_model extends CI_Model {
	...
	function sendEmail2($to, $sub, $msg){	
		...
		//$ci->email->set_header('–„ng k? t‡i kho?n', '–„ng k? th‡nh cÙng');
		$send = $ci->email->send();	
		return $send;
	}
	...
	function getButton() {
		...
		$str .= '<a class="btn btn-primary" href="'.base_url().'dang-nhap.html?r='.$redirectUrl.'">ƒêƒÉng nh·∫≠p</a> ';
		$str .= '<a class="btn btn-primary" href="'.base_url().'dang-ky.html">ƒêƒÉng k√Ω th√†nh vi√™n</a> ';
		$str .= '<a class="btn btn-primary" href="'.base_url().'dich-vu.html">ƒêƒÉng k√Ω d·ªãch v·ª•</a>';
		$str .= '</div>';
		return $str;
	}
