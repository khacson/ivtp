 ...
 * @copyright 2015
 */
class Supperlier extends CI_Controller {
    ...
    function _view() {
        ...
        if($action == ""){
            ...
            if($idaction > 0){
                $supperlier_check = $this->model->getSlitetop($idaction);
                $data->supperlier_name = $supperlier_check[0]->supperlier_name;
                $data->description = trim($supperlier_check[0]->description);
				$data->img = $supperlier_check[0]->img;
				$data->url = trim($supperlier_check[0]->url);	//print_r($data	);exit;			
            }
            else{//print_r($action);exit;
                $data->supperlier_name = "";
                $data->description = "";
				$data->img = "";
                $data->url = "";          				
            }
    ...
    function getList() {
        ...
        $data->datas = $query;
        $page_view = $this->admin->pagination($count, $numrows, 5, 'supperlier/', $page);
    ...
    function save() {
        ...
        if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
            ...
            /*$data = getimagesize(base_url().'files/supperlier/'.$image_data);
            $width = $data[0];
            $height = $data[1];
            if($width < 100){
                $result['status'] = 2;
    ...
    function resizeImg($image_data) {
        ...
        $configz['source_image'] = './files/supperlier/' . $image_data;
        $configz['new_image'] = './files/supperlier/' . $image_data;
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/supperlier/';
    ...
    function deletes() {
        ...
        $array['dateupdate'] = gmdate("Y-m-d H:i:s", time() + 7 * 3600);
        $array['userupdate'] = $login->username;
        $this->model->table('ivt_supperlier')->where('id', $id)->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
