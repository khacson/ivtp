	...
	<tr class="content edit" img = "<?=$item->img;?>" id="<?=$item->id; ?>" >
		...
		<td class="title">
		 <?php if (isset($permission['edit'])) { ?>
		   <a href="<?=admin_url();?>markettrend/form/<?=$item->id;?>"><?=$item->title;?></a>
		 <?php }else{?>
			<?=$item->title;?>
		 <?php }?>
		</td>
		
        <td class="text-center"><img src="<?php echo base_url().'files/markettrend/thumb/'.$item->image?>" alt="img" height="42" width="42"></td>	
